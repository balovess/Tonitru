...
/// Decodes a batch of U64 values from bytes.
pub fn decode_u64_batch(raw_value_slice: &[u8], count: usize) -> Result<Vec<u64>> {
    let required_len = count * mem::size_of::<u64>();
    if raw_value_slice.len() < required_len {
        return Err(Error::CodecError(format!(
            "Incomplete data for U64 batch decoding. Expected at least {} bytes, got {}",
            required_len,
            raw_value_slice.len()
        )));
    }
    ...
    #[cfg(all(target_arch = "x86_64", target_feature = "sse4.1"))]
    {
        if is_x86_feature_detected!("sse4.1") {
            ...
            while current_offset < required_len {
                // Use SIMD to load and extract u64 values
                ...
                // Extract up to 2 u64 values from the 128-bit register
                result.push(unsafe { _mm_extract_epi64(val_m128i, 0) as u64 });
                if count > result.len() {
                    result.push(unsafe { _mm_extract_epi64(val_m128i, 1) as u64 });
                }
...
mod tests {
    ...
    fn test_decode_u64() {
        ...
        let raw_value_slice_u64 = &encoded_u64[encoded_u64.len().checked_sub(8).unwrap()..]; // Length 8
    ...
    fn test_decode_u64_errors() {
        ...
        let result = decode_u64(7, &[0x00; 7]);
    ...
    fn test_decode_u64_batch() {
        ...
        let decoded = decode_u64_batch(&data, 3).unwrap();
        assert_eq!(decoded, expected);

        // Test with incomplete data
        let incomplete_data: Vec<u8> = vec![
            0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // 1
            0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Incomplete 2
        ];
        let result = decode_u64_batch(&incomplete_data, 2);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "Codec Error: Incomplete data for U64 batch decoding. Expected at least 16 bytes, got 15"
        );

        // Test with empty data
        let result = decode_u64_batch(&[], 0);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), vec![] as Vec<u64>);

         // Test with empty data and count > 0
        let result = decode_u64_batch(&[], 1);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "Codec Error: Incomplete data for U64 batch decoding. Expected at least 8 bytes, got 0"
        );
    }
}