...
pub fn decode_i32(length: u64, raw_value_slice: &[u8]) -> Result<HtlvValue> {
    ...
    if raw_value_slice.len() < mem::size_of::<i32>() {
         return Err(Error::CodecError("Incomplete data for I32 value".to_string()));
    }
    // Use from_le_bytes for standard decoding
    let mut bytes = [0u8; mem::size_of::<i32>()];
    bytes.copy_from_slice(&raw_value_slice[..mem::size_of::<i32>()]);
    Ok(HtlvValue::I32(i32::from_le_bytes(bytes)))

    // The SIMD part in the original decode_i32 was incorrect for a single value.
...
// Note: The previous `decode_i32_batch` function is now replaced by the `BatchDecoder` implementation below.

impl BatchDecoder for i32 {
    type DecodedType = i32;

    /// Decodes a batch of I32 values from bytes using zero-copy reinterpretation.
    ...
    fn decode_batch(data: &[u8]) -> Result<(&[Self::DecodedType], usize)> {
        ...
        let decoded_slice = unsafe {
            ...
            // 2. We check that `data.len()` is a multiple of `size_of::<i32>()`,
            //    ensuring the total size is correct for `count` i32 elements.
            // 3. We assume the data is in little-endian format, consistent with `i32::from_le_bytes`.
            ...
            //    the pointer to be aligned for `Self::DecodedType` (i32). `&[u8]` does not
            ...
            slice::from_raw_parts(data.as_ptr() as *const i32, count)
        };
...
mod tests {
    ...
    fn test_decode_i32() {
        ...
        // Assuming encode_item for I32 results in [Tag(varint), Type(u8), Length(varint), Value(i32)]
        // For tag 0 (1 byte), type I32 (1 byte), length 4 (1 byte), the header is 3 bytes.
        let raw_value_slice_i32 = &encoded_i32[3..]; // Length 4
        let decoded_i32 = decode_i32(4, raw_value_slice_i32).unwrap();
        assert_eq!(decoded_i32, HtlvValue::I32(-2147483648));

        // Test with incomplete data
        let result = decode_i32(4, &[0x00, 0x00, 0x00]);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "Codec Error: Incomplete data for I32 value"
        );
    }
    ...
    fn test_decode_i32_errors() {
        ...
        let result = decode_i32(3, &[0x00, 0x00, 0x00, 0x00]); // Provide enough data but wrong length
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
    ...
    #[test]
    fn test_decode_batch_i32() {
        // Test decoding a batch of I32 values
        let values: Vec<i32> = vec![-1, -2, 1, 2, 0];
        // Reinterpret the i32 vector as a u8 slice. This ensures correct alignment.
        let data: &[u8] = unsafe {
            slice::from_raw_parts(values.as_ptr() as *const u8, values.len() * mem::size_of::<i32>())
        };
        let expected: &[i32] = &[-1, -2, 1, 2, 0];
        let (decoded_slice, bytes_consumed) = i32::decode_batch(&data).unwrap();
        ...
        assert_eq!(bytes_consumed, 0);
         ...
         // Test with incomplete data (not a multiple of size_of::<i32>()) - This test case is still valid
        let incomplete_data: &[u8] = &[0x01, 0x00, 0x00]; // 3 bytes
        let result = i32::decode_batch(&incomplete_data);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err().to_string(),
            "Codec Error: Invalid data length for I32 batch decoding. Length (3) must be a multiple of 4"
        );
    }
}